    ...
    <section
      ...
      <div className='grid [grid-template-columns:repeat(3,fit-content(300px))] gap-8 justify-center'>
        {fake_data.map((product: IProduct, index) => (
          <div
            key={index}
            className='relative flex flex-col items-center justify-between py-4  rounded-lg shadow-neutral gap-4 w-[18em] bg-white'>
            <svg
              id='heart-svg'
              ...
              className='feather feather-heart h-8 w-8 absolute right-3 top-3 cursor-pointer duration-1000'
            ...
            <span className='flex items-center font-extrabold text-2xl text-base-font-300 [letter-spacing:-1px]'>
              ...
              {product.rating.rate < 3.5 && (
                <span className='absolute top-2 left-2 text-[0.7em] flex items-center gap-1 border rounded-md px-3 font-medium bg-[#ffffff1f] [backdrop-filter:blur(5px)]'>
                  Oferta
                  <FaFire className='text-red-500' />
                </span>
              )}
            </span>
            <button className='btn_secondary flex items-center gap-2 '>
              Sumar al carro
